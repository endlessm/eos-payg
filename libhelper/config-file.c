 ...
 *
 * When queried for keys, an #HlpConfigFile instance will return the value from
 ...
 *
 * When listing groups, an #HlpConfigFile will return the deduplicated union
...
G_DEFINE_TYPE (HlpConfigFile, hlp_config_file, G_TYPE_OBJECT)
...
typedef enum
{
  ...
  PROP_DEFAULT_PATH,
} HlpConfigFileProperty;

static GParamSpec *props[PROP_DEFAULT_PATH + 1] = { NULL, };

static void
hlp_config_file_init (HlpConfigFile *self)
{
  self->key_files = g_ptr_array_new_with_free_func ((GDestroyNotify) g_key_file_unref);
}

static void
hlp_config_file_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *spec)
{
  HlpConfigFile *self = HLP_CONFIG_FILE (object);

  switch ((HlpConfigFileProperty) property_id)
    {
    case PROP_PATHS:
...
static void
hlp_config_file_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *spec)
{
...
static void
hlp_config_file_constructed (GObject *object)
{
  HlpConfigFile *self = HLP_CONFIG_FILE (object);
  ...
  /* Chain up. */
  G_OBJECT_CLASS (hlp_config_file_parent_class)->constructed (object);
...
static void
hlp_config_file_finalize (GObject *object)
{
  HlpConfigFile *self = HLP_CONFIG_FILE (object);
  ...
  G_OBJECT_CLASS (hlp_config_file_parent_class)->finalize (object);
}

static void
hlp_config_file_class_init (HlpConfigFileClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = hlp_config_file_constructed;
  object_class->finalize = hlp_config_file_finalize;
  object_class->get_property = hlp_config_file_get_property;
  object_class->set_property = hlp_config_file_set_property;
  ...
  /**
   * HlpConfigFile:default-resource:
   *
  ...
  /**
   * HlpConfigFile:default-path:
   *
   * Path to the default configuration file in #HlpConfigFile:default-resource.
   *
   * Since: UNRELEASED
   */
 ...
 *
 * Create a new #HlpConfigFile representing the configuration loaded from the
 ...
 *
 * Returns: (transfer full): a newly allocated #HlpConfigFile
 * Since: UNRELEASED
 */
HlpConfigFile *
hlp_config_file_new (const gchar * const *key_file_paths,
                     GResource           *default_resource,
                     const gchar         *default_path)
{
  ...
  return g_object_new (HLP_TYPE_CONFIG_FILE,
...
static gboolean
hlp_config_file_ensure_loaded (HlpConfigFile  *self,
                               gsize           idx,
...
static gboolean
hlp_config_file_get_file_for_key (HlpConfigFile  *self,
                                  ...
                                  GError        **error)
{
   ...
   * hlp_config_file_ensure_loaded() which loads the default config file. */
  for (i = 0; i <= self->n_paths; i++)
    {
      path = (i < self->n_paths) ? self->paths[i] : self->default_path;

      if (!hlp_config_file_ensure_loaded (self, i, &key_file, error))
        return FALSE;
...
/**
 * hlp_config_file_get_uint:
 * @self: an #HlpConfigFile
 ...
 */
guint
hlp_config_file_get_uint (HlpConfigFile  *self,
                          const gchar    *group_name,
                          const gchar    *key_name,
                          guint           min_value,
                          guint           max_value,
                          GError        **error)
{
  ...
  g_return_val_if_fail (HLP_IS_CONFIG_FILE (self), 0);
  ...
  if (!hlp_config_file_get_file_for_key (self, group_name, key_name, &key_file, &path, error))
    return 0;
 ...
 */
gboolean
hlp_config_file_get_boolean (HlpConfigFile  *self,
                             const gchar    *group_name,
                             const gchar    *key_name,
                             GError        **error)
{
  ...
  g_return_val_if_fail (HLP_IS_CONFIG_FILE (self), FALSE);
  g_return_val_if_fail (group_name != NULL, FALSE);
  g_return_val_if_fail (key_name != NULL, FALSE);
  g_return_val_if_fail (error == NULL || *error == NULL, FALSE);

  if (!hlp_config_file_get_file_for_key (self, group_name, key_name, &key_file, NULL, error))
    return FALSE;
  g_assert (key_file != NULL);
 ...
 */
gchar *
hlp_config_file_get_string (HlpConfigFile  *self,
                            const gchar    *group_name,
                            const gchar    *key_name,
                            GError        **error)
{
  ...
  g_return_val_if_fail (HLP_IS_CONFIG_FILE (self), NULL);
 ...
 */
gchar **
hlp_config_file_get_strv (HlpConfigFile  *self,
                          const gchar    *group_name,
                          const gchar    *key_name,
 ...
 */
gchar **
hlp_config_file_get_groups (HlpConfigFile  *self,
                            gsize          *n_groups_out,
                            GError        **error)
{
  gsize i;
