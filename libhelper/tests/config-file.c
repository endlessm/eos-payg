...
#include <libhelper/tests/config-file-resources.h>
#include <locale.h>
#include <string.h>
#include <sys/types.h>
#include <unistd.h>
       ...
       gconstpointer  user_data G_GNUC_UNUSED)
{
  ...
  fixture->tmp_dir = g_dir_make_tmp ("helper-tests-config-XXXXXX", &error);
  ...
  fixture->default_resource = config_file_resources_get_resource ();
  fixture->default_path = "/com/endlessm/Helper/config/config-test.conf";
  fixture->default_path_invalid = "/com/endlessm/Helper/config/config-test-invalid.conf";
}
...
{
  g_autoptr(GError) error = NULL;
  g_autoptr(HlpConfigFile) config = NULL;
  ...
  config = hlp_config_file_new (paths, fixture->default_resource, fixture->default_path);

  loaded_file = hlp_config_file_get_uint (config, "Test", "File", 0, G_MAXUINT, &error);
  g_assert_no_error (error);
                             ...
                             gconstpointer  user_data G_GNUC_UNUSED)
{
  ...
  hlp_config_file_get_uint (config, "Any", "Thing", 0, G_MAXUINT, &error);
  ...
  if (g_test_subprocess ())
    {
      ...
      config = hlp_config_file_new (paths, fixture->default_resource,
  ...
  else
    {
      ...
      g_test_trap_assert_stderr ("*ERROR*hlp_config_file_constructed: "
                                   ...
                                   gconstpointer  user_data G_GNUC_UNUSED)
{
  ...
  file1_key = hlp_config_file_get_boolean (config, "Test", "File1", &error);
  g_assert_no_error (error);
  g_assert_true (file1_key);

  file2_key = hlp_config_file_get_boolean (config, "Test", "File2", &error);
  g_assert_no_error (error);
                         ...
                         gconstpointer  user_data G_GNUC_UNUSED)
{
  ...
  groups = hlp_config_file_get_groups (config, &n_groups, &error);
  g_assert_no_error (error);
